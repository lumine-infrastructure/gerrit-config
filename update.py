...
github_projects = {x.full_name for x in github.get_organization(GITHUB_ORG).get_repos()}
gerrit_projects = set()
...
for repo in missing:
    ...
    github.get_organization(GITHUB_ORG).create_repo(repo.replace(f"{GITHUB_ORG}/", ""), has_wiki=False, has_downloads=False, has_projects=False, has_issues=False, private=False)
    github.get_organization(GITHUB_ORG).get_repo(repo.replace(f"{GITHUB_ORG}/", "")).edit(has_pull_requests=False)
